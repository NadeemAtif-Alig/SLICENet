    ...
    if not args.best:
        if args.checkpoint:
            if os.path.isfile(args.checkpoint):
                ...
                model.load_state_dict(checkpoint['model'], strict=False)  # this is the original, use it
                #model.load_state_dict(checkpoint)
                print("########### save model #####################")
                torch.save(model, "./model_7599.pkl")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="slicenet", help="model name ")
    ...
    parser.add_argument('--checkpoint', type=str,
                        default="./model/path/of/the/trained/model/model_1000.pth",
                        help="use the file to load the checkpoint for evaluating or testing ")
    parser.add_argument('--save_seg_dir', type=str, default="./result/",
                        help="saving path of prediction result")
    parser.add_argument('--best', action='store_true', help="Get the best result among last few checkpoints")  # This line for visualization
    parser.add_argument('--save', action='store_false', help="Save the predicted image")  # This line for visualization
